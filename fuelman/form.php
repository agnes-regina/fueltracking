...
document.addEventListener('DOMContentLoaded', function() {
    // Foto segel awal
    for (let i = 1; i <= 4; i++) {
        const input = document.getElementById('fm_segel_photo_awal_' + i);
        if (input) {
            input.onchange = function() {
                compressAndUpload(this, 'preview_segel_awal_' + i);
            };
        }
    }
    // Foto tanki kosong
    for (let i = 1; i <= 4; i++) {
        const input = document.getElementById('fm_photo_akhir_' + i);
        if (input) {
            input.onchange = function() {
                compressAndUpload(this, 'preview_photo_akhir_' + i);
            };
        }
    }
    // Foto kejernihan
    const kejernihanInput = document.getElementById('fm_photo_kejernihan');
    if (kejernihanInput) {
        kejernihanInput.onchange = function() {
            compressAndUpload(this, 'preview_kejernihan');
        };
    }
});
</script>
